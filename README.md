...
| Filtr górnoprzepustowy (HPF) | C = 22 µF, dwa stopnie kaskadowe → **−1,4 dB @ 20 Hz** (stały, S3/S4 zachowane; przy 10 µF byłoby −4,9 dB — niedopuszczalne) |
...
zaledwie −1,4 dB (kaskada dwóch stopni) — zachowuje zarówno S4 (20–30 Hz), jak i S3 (25–50 Hz).
...
### Analiza HPF wynikająca z netlistu

Połączenia 4–5 i 9–10 tworzą dwa kaskadowe filtry górnoprzepustowe (HPF):

| HPF | Połączenia | Kondensator | Rezystor | f_c |
|---|---|---|---|---|
| Stopień 1 (wejście U1A) | 4–5 | C_in = 22µF | R_in1 = 909Ω | **7,96 Hz** |
| Stopień 2 (wejście U1B) | 9–10 | C_inter = 22µF | R_in2 = 909Ω | **7,96 Hz** |

Oba stopnie identyczne → kaskada dwóch HPF jednobiegunowych o tej samej f_c:

```
f_-3dB_kaskada = f_c × √(2^(1/n) − 1)  dla n=2 → f_c × √(√2 − 1) ≈ f_c × 0,6436
f_-3dB = 7,96 Hz × ... → łączny punkt −3dB ≈ 11,3 Hz

Tłumienie @ 20 Hz:
H(20Hz) = [20 / √(20² + 7,96²)]² = (0,929)² = 0,864 → −1,4 dB
```

| Częstotliwość | Tłumienie | Ocena |
|---|---|---|
| 20 Hz (S4 dolna granica) | **−1,4 dB** | zachowane ✓ |
| 25 Hz (S4) | −0,8 dB | zachowane ✓ |
| 40 Hz (S3/S4 główne) | −0,3 dB | minimalne ✓ |
| 100 Hz (S1/S2) | <−0,05 dB | pomijalne ✓ |

> **Tłumienie jest stałe** — bez trymera nie ma żadnego elementu zmieniającego f_c
> w trakcie nagrania. Wartość −1,4 dB @ 20 Hz jest deterministyczna i powtarzalna.
> Przy C=10µF (błędny dobór) kaskada daje −4,9 dB @ 20 Hz — S4 poważnie ucięte.
...
- [x] Filtr HPF dobrany pod tony S3/S4 (C=22µF, kaskada −1,4 dB @ 20Hz, f_-3dB≈11,3Hz)
...
#### Analiza HPF wynikająca z netlistu (Opcja 2)

Połączenia 4–5 i 9–10 — identyczne jak w Opcji 1 (rdzeń wspólny):

| HPF | Połączenia | Kondensator | Rezystor | f_c |
|---|---|---|---|---|
| Stopień 1 (wejście U1A) | 4–5 | C_in = 22µF | R_in1 = 909Ω | **7,96 Hz** |
| Stopień 2 (wejście U1B) | 9–10 | C_inter = 22µF | R_in2 = 909Ω | **7,96 Hz** |
| **Kaskada łączna** | | | f_-3dB ≈ **11,3 Hz** | **−1,4 dB @ 20 Hz** |

Dodatkowy HPF wyjścia zbalansowanego (Opcja 2 specyficzny):

| HPF | Połączenia | Kondensator | Rezystor R_ser | f_c |
|---|---|---|---|---|
| Wyjście HOT | 19–20 | C_hot = 10µF/63V | R_ser_hot = 100Ω | 159 Hz (bez obciążenia) |
| Wyjście COLD | 25–26 | C_cold = 10µF/63V | R_ser_cold = 100Ω | 159 Hz (bez obciążenia) |

> C_hot/C_cold przy typowym obciążeniu interfejsu (R_in_interfejsu ≈ 10kΩ):
> f_c = 1/(2π × 10µF × (100+10000)) = **1,6 Hz** → tłumienie @ 20 Hz: **−0,01 dB**.
> Wartość 159 Hz dotyczy tylko pracy bez podłączonego interfejsu — nie jest stanem roboczym.
> **63V rating C_hot/C_cold obowiązkowo** — na kondensatorach ~26V DC z phantom (szczegóły w sekcji Opcji 2).
